...
Usage examples:
  ...
  uv run python scripts/diagnose_wol.py --probe --url http://powerspec:7997
  uv run python scripts/diagnose_wol.py --show-config --env-file /path/to/.env
  uv run python scripts/diagnose_wol.py --probe --url http://h1:7997,http://h2:7997 \\
...
def _suggest_wol_errors(exc: Exception, color: bool) -> None:
    ...
    if "permission" in msg or "operation not permitted" in msg:
        ...
        print(f"  {_info('On Linux try: sudo python scripts/diagnose_wol.py --send-wol', color)}")
