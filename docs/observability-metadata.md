...
|-------|------|-------------|---------|
| `session_id` | string | Client-provided identifier grouping multiple API calls in a session | `"12"`, `"session-abc-123"` |
| `process_id` | string | ID for a single task/operation, which may span multiple requests across services (auto-generated by middleware if absent, preserved if forwarded from an earlier hop of the same task) | `"a1b2c3d4-e5f6-..."` |
...
> **Note:** `process_id` does not need to be supplied when starting a new task — the server middleware (`FromContextMiddleware` / `inject_from_logging_context`) auto-generates one and returns it in the `X-Process-ID` response header. If this call *continues* a task that started elsewhere (e.g. an upstream service forwarding its own `process_id`), include it in `from` and the middleware will preserve it rather than generating a new one, so the whole task shares one id across services. For non-HTTP contexts (MCP tools, background workers), use `from_dict.setdefault("process_id", generate_process_id())` from `core_lib` — same inherit-or-generate behavior.
...
### 1. Session vs. Process Tracking

- **`session_id`** (client-provided): Groups multiple tasks belonging to the same user session/conversation. Supply the same value across related requests.
- **`process_id`**: Identifies a single task/operation. Auto-generated by the middleware if the caller doesn't supply one; if a task spans multiple services, forward the same `process_id` to each downstream call so all of them share it. Returned in the `X-Process-ID` response header.
...
# Each independent question is its own task and gets its own process_id
# automatically (do not reuse one process_id across unrelated questions)
for question in questions:
    ...
    # response.headers["X-Process-ID"] contains this task's process_id —
    # forward it in `from_["process_id"]` on any follow-up call that's part
    # of the same task
```
