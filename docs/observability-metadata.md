...
| `process_id` | string | **Server-generated** unique ID for a single request/process (auto-injected by middleware) | `"a1b2c3d4-e5f6-..."` |
...
> **Note:** `process_id` does not need to be supplied by the client. It is automatically generated by the server middleware (`FromContextMiddleware` / `inject_from_logging_context`) for each HTTP request and returned in the `X-Process-ID` response header. For non-HTTP contexts (MCP tools, background workers), use `generate_process_id()` from `core_lib`.
...
- **`session_id`** (client-provided): Groups multiple API calls belonging to the same conversation or workflow. Supply the same value across related requests.
- **`process_id`** (server-generated): Automatically created per request by the middleware. Correlates all log records for a single API call. Returned in the `X-Process-ID` response header.
...
# Use in all related requests — each request gets its own process_id automatically
for question in questions:
    ...
    # response.headers["X-Process-ID"] contains the unique process_id for this call
```
