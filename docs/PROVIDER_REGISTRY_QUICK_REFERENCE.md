...
providers:
  ...
  - provider: mistral             # Mistral AI — OpenAI-compatible endpoint
    ...
    model: mistral-large-latest   # or magistral-medium-latest for reasoning
    priority: 3
    tier: standard

  - provider: ovh                 # OVH AI Endpoints — OpenAI-compatible (auto-sets base URL)
    api_key: ${OVH_AI_ENDPOINTS_TOKEN}
    model: Qwen3.5-14B
    priority: 4
    thinking:
      enabled: false              # Disable thinking tokens (/no_think prefix)
    tier: standard
...
|-------|------|----------|-------------|
| `provider` | str | Yes | `gemini`, `openai`, `openai-responses`, `openrouter`, `alibaba`, `azure-openai`, `ollama`, `mistral`, `ovh` |
...
- `model`: Mistral model name, e.g. `mistral-small-latest`, `mistral-large-latest`, `magistral-medium-latest`
- `thinking` / `thinking_enabled`: `true` — enables `reasoning_effort` for `magistral-*` reasoning models only; ignored for all other Mistral models

**OVH AI Endpoints (`ovh` / `ovhcloud`):**
- `api_key`: OVH AI Endpoints token
- `model`: Model name deployed on your endpoint (e.g. `Qwen3.5-14B`)
- `host`: Override the base URL (default: `https://oai.endpoints.kepler.ai.cloud.ovh.net/v1`)
- `thinking` / `thinking_config.enabled: false`: Disables Qwen3 thinking tokens by prepending `/no_think` to the system message
- No `response_format` is sent for structured output — a JSON schema hint is injected into the system message instead
