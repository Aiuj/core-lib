...
- **session_id**: Client-provided identifier for one user session, spanning many tasks/requests
- **process_id**: Identifier for a single task/operation (e.g. one document ingestion, one RFx run, one question answered), which may itself span multiple API calls across services. Auto-generated by middleware if not supplied; if a caller forwards a `process_id` it already has (because this call continues a task started elsewhere), the middleware preserves it instead of generating a new one — do not supply one for a *new* task, but do forward an inherited one
...
> **Note:** When using `FromContextMiddleware` or `inject_from_logging_context`, a `process_id` is ensured for the duration of the request and returned in the `X-Process-ID` response header — a freshly generated one if none was supplied, or the inherited one if the caller forwarded a `process_id` from an earlier hop in the same task. This enables correlating all logs for a single task, even across services.
