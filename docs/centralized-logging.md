...
> **Note:** `process_id` identifies one task (which may span multiple API calls across services). The middleware auto-generates one if none is present, so you don't need to supply it when starting a new task -- but if this call continues a task that started elsewhere (an upstream service forwarding its own `process_id`), include it in `from` so the middleware preserves it instead of generating a new one.
```
...
|------------|----------------|-------------|
| `process_id` | `process.id` | ID for one task (auto-generated by middleware if absent, preserved if forwarded from an earlier hop of the same task) |
| `session_id` | `session.id` | Client-provided session identifier (groups multiple tasks) |
...
```
# Find all logs for a single task, even across services (process_id is
# auto-generated at the task's origin and forwarded unchanged by every hop
# that continues it)
process.id = "a1b2c3d4-e5f6-7890-abcd-ef1234567890"

# Find all logs for a user session (groups multiple tasks)
...
> **Tip:** Use `process.id` to isolate logs for a single task, even when that task spans several services. Use `session.id` to see all tasks in a user session. The `process_id` is also returned in the `X-Process-ID` response header for client-side correlation.

### FastAPI Middleware (Recommended)

Use the built-in middleware, which ensures every request has a `process_id`:
...
# 2. Ensures a process_id is present -- generating one if the caller didn't
#    forward one from an earlier hop of the same task
...
For non-HTTP contexts (MCP tools, background workers), ensure `process_id` is present manually. Use `setdefault` rather than an unconditional overwrite, so a `process_id` forwarded from the caller (continuing a task started elsewhere) is preserved instead of replaced:
...
async def call_tool(name: str, arguments: dict, from_: str | None = None):
    from_dict = parse_from(from_)
    from_dict.setdefault("process_id", generate_process_id())  # ID for this task, inherited or new
